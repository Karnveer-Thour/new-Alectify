...
@Entity('master_preventive_maintenance_files')
export class MasterPreventiveMaintenanceFiles extends BaseEntity<MasterPreventiveMaintenanceFiles> {
  ...
  static setSignUrls(shouldSign: boolean) {
    MasterPreventiveMaintenanceFiles.signUrls = shouldSign;
  }

  // After entity is loaded from DB, this will run if `signUrls` is true
  @AfterLoad()
  async populateSignedURLs() {
    if (MasterPreventiveMaintenanceFiles.signUrls && this.filePath) {
  ...
  constructor(entity: Partial<MasterPreventiveMaintenanceFiles>) {
    super(entity);
