...
export class timelineViewUpdateForMultiAssetsAreasMigration1736353180169
  implements MigrationInterface
{
  name = 'timelineViewUpdateForMultiAssetsAreasMigration1736353180169';
...
`);
  ...
  preventive_maintenances.id as id, pm_type::text as type, work_title as name, pmas.asset_id as asset_id, pmar.area_id as area_id, status, due_date, detail, null as quantity, null as user_id, null as user_first_name, null as user_last_name, null as user_email, preventive_maintenances.created_at, preventive_maintenances.updated_at, COUNT(comments.id) as comments_count  
  FROM preventive_maintenances left join comments on preventive_maintenances.id::text = comments.reference_id LEFT JOIN preventive_maintenance_areas pmar ON pmar.preventive_maintenance_id = preventive_maintenances.id
  LEFT JOIN preventive_maintenance_assets pmas ON pmas.preventive_maintenance_id = preventive_maintenances.id where is_future = false group by preventive_maintenances.id, pmas.asset_id, pmar.area_id 
  UNION
