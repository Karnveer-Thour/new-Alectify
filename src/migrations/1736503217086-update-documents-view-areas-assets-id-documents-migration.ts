...
export class updateDocumentsViewAreasAssetsIdDocumentsMigration1736503217086
  implements MigrationInterface
{
  name = 'updateDocumentsViewAreasAssetsIdDocumentsMigration1736503217086';
...
UNION
 SELECT pmd.id,
   ...
   FROM preventive_maintenance_documents pmd
     ...
     LEFT JOIN authentication_user ON pmd.user_id = authentication_user.id
     LEFT JOIN authentication_user deleted_by_authentication_user ON pmd.deleted_by = deleted_by_authentication_user.id
     LEFT JOIN preventive_maintenance_areas pmar ON pmar.preventive_maintenance_id = pmd.id
     LEFT JOIN preventive_maintenance_assets pmas ON pmas.preventive_maintenance_id = pmd.id;`);
    await queryRunner.query(
      `INSERT INTO "typeorm_metadata"("database", "schema", "table", "type", "name", "value") VALUES (DEFAULT, $1, DEFAULT, $2, $3, $4)`,
      [
        'public',
